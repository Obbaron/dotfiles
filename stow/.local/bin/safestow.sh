...
warn_if_dirty() {
    ...
    warned=1
    if [ "$mutating" = 1 ]; then
        echo "safestow: aborted after backups began; .bak files may exist and stowing may be incomplete." >&2
    fi
    return 0
}
...
if [ -z "$conflicts" ]; then
    [ "$status" -eq 0 ] || exit "$status"   # stow already explained the failure above
